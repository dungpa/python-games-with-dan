...
dinosaur = Actor("haxoreset")
dinosaur.pos = CENTRE_X + 5, CENTRE_Y - 40
...
def update_dinosaur(move):
    global game_over
    if not game_over:
        if move == 0:
            up.image = "up-lit"
            dinosaur.image = "haxoup"
            clock.schedule(reset_dinosaur, 0.5)
        elif move == 1:
            right.image = "right-lit"
            dinosaur.image = "haxoright"
            clock.schedule(reset_dinosaur, 0.5)
        elif move == 2:
            down.image = "down-lit"
            dinosaur.image = "haxodown"
            clock.schedule(reset_dinosaur, 0.5)
        else:
            left.image = "left-lit"
            dinosaur.image = "haxoleft"
            clock.schedule(reset_dinosaur, 0.5)
    return        
...
def update():
    ...
    if not game_over:
        ...
        sounds.vanishinghorizon.stop()
            
generate_moves()  
sounds.vanishinghorizon.play(loops=-1)
